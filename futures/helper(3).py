...
def parLapply(CORE_NUM, iterable, func, *args, **kwargs):
    with dask.config.set(scheduler='processes', num_workers=CORE_NUM):
        ...
        result = compute([delayed(f_par)(item) ffor item in iterable])[0]
        return result
...
from collections import OrderedDict
def get_signal_pnl(file, product, signal_name, thre_mat, reverse=1, tranct=1.1e-4, max_spread=0.61, tranct_ratio=True, HEAD_PATH="d:/intern", SIGNAL_PATH = "d:/intern", atr_filter=0, rebate=0):
    ## load data
...
#     last_ask = data["next.ask"][-10:]
#     last_bid = data["next.bid"][-10:]
#     data["next.ask"][-10:][last_ask<1] = last_bid+max_spread-0.1
#     data["next.bid"][-10:][last_bid<1] = last_ask-max_spread+0.1
...
def evaluate_signal(signal, all_dates, product, min_pnl, min_num, 
    ...
    if reverse>=0:
        ...
        trend_signal_stat = get_signal_stat(signal_name, thre_mat, product, all_dates, CORE_NUM, split_str="2018", reverse=1, tranct=tranct, 
                                   ...
                                   HEAD_PATH=HEAD_PATH, SIGNAL_PATH=SIGNAL_PATH)
    if reverse<=0:
        print("reverse=-1")
